...
def update_shermor(N, delta, rng, hs, Gu, Gd):
    ...
    for i in shuffle(rng, N):
        ...
        pu = 1.0 + (1.0 - Gu[i, i]) * delu
        pd = 1.0 + (1.0 - Gd[i, i]) * deld
        p = pu * pd
...
def measure_eqlt(params, sign, Gu, Gd, meas):
    ...
    for i in range(params["N"]):
        r = params["map_i"][i]
        pre = sign / params["degen_i"]
        Guii = Gu[i, i]
        Gdii = Gd[i, i]
        meas["density"][r] += pre * (2.0 - Guii - Gdii)
        meas["double_occ"][r] += pre * (1.0 - Guii) * (1.0 - Gdii)
    ...
    for state["sweep"] in range(state["sweep"], params["n_sweep"]):
        ...
        if state["sweep"] % 2 == 0:  # up sweep from l=0 to l=L-1
            for l in range(L):
                ...
                sign *= update_shermor(
                    params["N"], params["del"], state["rng"], state["hs"][l], Gttu, Gttd
                )
                ...
                Bd[l] = params["exp_Kd"] * exp_Vd
                iBu[l] = (
                    exp_Vd[None].T * params["inv_exp_Ku"]
                )  # note that exp_Vd = 1/exp_Vu
                ...
                if m == n_matmul - 1:  # calculate accurately G(l+1, l+1)
                    QdX0u[f] = (
                        calc_QdX_first(Cu[f])
                        if f == 0
                        else calc_QdX(Cu[f], QdX0u[f - 1])
                    )
                    Gttu[...], signu = (
                        calc_Gtt_last(QdX0u[f])
                        if f == F - 1
                        else calc_Gtt(QdX0u[f], QdXLu[f + 1])
                    )
                    QdX0d[f] = (
                        calc_QdX_first(Cd[f])
                        if f == 0
                        else calc_QdX(Cd[f], QdX0d[f - 1])
                    )
                    Gttd[...], signd = (
                        calc_Gtt_last(QdX0d[f])
                        if f == F - 1
                        else calc_Gtt(QdX0d[f], QdXLd[f + 1])
                    )
                    sign = signu * signd
                else:  # wrap forward
                ...
                if enabled_eqlt and (l + 1) % params["period_eqlt"] == 0:
                    Gu_half = np.dot(
                        params["inv_exp_halfKu"], np.dot(Gttu, params["exp_halfKu"])
                    )
                    Gd_half = np.dot(
                        params["inv_exp_halfKd"], np.dot(Gttd, params["exp_halfKd"])
                    )
            ...
            for l in reversed(range(L)):
                ...
                if m == 0:  # recalculate accurately G(l, l)
                    QdXLu[f] = (
                        calc_QdX_first(Cu[f].T)
                        if f == F - 1
                        else calc_QdX(Cu[f].T, QdXLu[f + 1])
                    )
                    Gttu_new, signu = (
                        calc_Gtt_last(QdXLu[f], transpose=True)
                        if f == 0
                        else calc_Gtt(QdX0u[f - 1], QdXLu[f])
                    )
                    QdXLd[f] = (
                        calc_QdX_first(Cd[f].T)
                        if f == F - 1
                        else calc_QdX(Cd[f].T, QdXLd[f + 1])
                    )
                    Gttd_new, signd = (
                        calc_Gtt_last(QdXLd[f], transpose=True)
                        if f == 0
                        else calc_Gtt(QdX0d[f - 1], QdXLd[f])
                    )
                    max_diff = max(
                        np.max(np.abs(Gttu - Gttu_new)), np.max(np.abs(Gttd - Gttd_new))
                    )
                    if max_diff > 1e-7:
                        print(
                            f"On sweep {state['sweep']}, G differed by {max_diff:g}. Consider increasing n_matmul."
                        )
                    Gttu[...] = Gttu_new
                    Gttd[...] = Gttd_new
                    sign = signu * signd
