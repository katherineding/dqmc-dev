...
def calc_eq_g(l, B):
    ...
    Q, jpvt, tau, work, info = dgeqp3(
        B[order[0]] if L % 2 == 1 else np.dot(B[order[1]], B[order[0]])
    )
    d = Q.diagonal().copy()
    d[d == 0.0] = 1.0
    ...
    for m in range((1 if L % 2 == 1 else 2), L, 2):
        W, work, info = dormqr(
            "R", "N", Q, tau, np.dot(B[order[m + 1]], B[order[m]]), work.shape[0]
        )
        W *= d
...
def dqmc(params, state, meas_eqlt, meas_uneqlt):
    ...
    for state["sweep"] in range(state["sweep"], params["n_sweep"]):
        ...
        for l in range(params["L"]):
            # updates
            sign *= update_shermor(
                params["N"], params["del"], state["rng"], state["hs"][l], Gu, Gd
            )
            ...
            if enabled_eqlt and (l + 1) % params["period_eqlt"] == 0:
                Gu_half = np.dot(
                    params["inv_exp_halfKu"], np.dot(Gu, params["exp_halfKu"])
                )
                Gd_half = np.dot(
                    params["inv_exp_halfKd"], np.dot(Gd, params["exp_halfKd"])
                )
