...
#pragma omp parallel for num_threads(OMP_MEAS_NUM_THREADS)
  ...
  if (meas_bond_corr || meas_thermal || meas_2bond_corr) {
    ...
    const int degen_bb = p->degen_bb;
    const int degen_bb2 = p->degen_bb2;
    const int degen_b2b2 = p->degen_b2b2;
    const int degen_b2b = p->degen_b2b;

#pragma omp target data map(                                                                     \
        tofrom : _pair_bb[ : num_bb * L], _jj[ : num_bb * L], _jnj[ : num_bb * L],               \
            _jjn[ : num_bb * L], _jnjn[ : num_bb * L], _jsjs[ : num_bb * L], _kk[ : num_bb * L], \
            _ksks[ : num_bb * L], _j2jn[ : num_b2b * L], _j2j[ : num_b2b * L],                   \
            _j2j2[ : num_b2b2 * L], _pair_b2b2[ : num_b2b2 * L], _js2js2[ : num_b2b2 * L],       \
            _k2k2[ : num_b2b2 * L], _ks2ks2[ : num_b2b2 * L], _jnj2[ : num_bb2 * L],             \
            _jj2[ : num_bb2 * L])                                                                \
    map(to : _peierlsu[ : N * N], _peierlsd[ : N * N], _pp_u[ : N * num_b2 / N],                 \
            _pp_d[ : N * num_b2 / N], _ppr_u[ : N * num_b2 / N], _ppr_d[ : N * num_b2 / N],      \
            _bonds[ : num_b * 2], _bond2s[ : num_b2 * 2], _map_bb[ : num_b * num_b],             \
            _map_b2b[ : num_b2 * num_b], _map_bb2[ : num_b2 * num_b],                            \
            _map_b2b2[ : num_b2 * num_b2], phase, Gutt[ : N * N * L],         \
            Gut0[ : N * N * L], Gu0t[ : N * N * L], Gdtt[ : N * N * L], Gdt0[ : N * N * L],      \
            Gd0t[ : N * N * L])
    {
...
#pragma omp loop  // this internal loop can be parallel
          for (int b = 0; b < num_b; b++) {
            ...
            if (meas_bond_corr) {
              _pair_bb[bb + num_bb * t] +=
                  0.5 * pre *
                  (gui0j0 * gdi1j1 + gui1j0 * gdi0j1 + gui0j1 * gdi1j0 + gui1j1 * gdi0j0);
              const num x = pui0i1 * puj0j1 * (delta_i0j1 - guj1i0) * gui1j0 +
                            pui1i0 * puj1j0 * (delta_i1j0 - guj0i1) * gui0j1 +
                            pdi0i1 * pdj0j1 * (delta_i0j1 - gdj1i0) * gdi1j0 +
                            pdi1i0 * pdj1j0 * (delta_i1j0 - gdj0i1) * gdi0j1;
            ...
            if (meas_thermal) {
              ...
              const num c3 =
                  ((delta_i0j0 - gdj0i0) * gdi0j0 + (delta_i1j0 - gdj0i1) * gdi1j0 +
...
#pragma omp loop
            for (int b = 0; b < num_b2; b++) {
              ...
              const num pre = phase / degen_b2b2;
              ...
              _pair_b2b2[bb + num_b2b2 * t] +=
                  0.5 * pre *
                  (gui0j0 * gdi2j2 + gui2j0 * gdi0j2 + gui0j2 * gdi2j0 + gui2j2 * gdi0j0);
