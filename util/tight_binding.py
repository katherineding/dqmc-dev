...
# magnetic field.
# nflux must be integer
def H_periodic_2D_EDWIN(Ny, Nx, ty=1.0, tx=1.0, tp=0.0, typp=0.0, txpp=0.0, \
    nflux=0, disorder = 0):
    B = nflux/(Ny*Nx)
    ...
    for iy in range(Ny):
        for ix in range(Nx):
            ...
            tij[ix + Nx*iy, ix + Nx*iy] += np.random.uniform(low=-disorder/2,high=disorder/2)
    #BEWARE
    #K[i,j] on paper is K[j,i] in code here because python is row major and 
    #C code is column major
    assert np.linalg.norm(tij- tij.T.conj()) < 1e-10
    
    return np.transpose(tij)
...
def JX_periodic_2D(Ny, Nx, ty=1.0, tx=1.0, tp=0.0, typp=0.0, txpp=0.0, nflux=0):
    ...
    #print("K^dagger - K norm:", np.linalg.norm(mat_ij.T - mat_ij.conj()))
