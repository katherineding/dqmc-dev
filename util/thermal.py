...
def j2j2_jxding(path):
    ...
    accum = 0;
    if nflux == 0:  
        pre_array= [1+2*tp**2, 1+2*tp**2, 2, 2, 2*tp,2*tp,2*tp,2*tp,4*tp,4*tp,tp**2,tp**2]
    else: 
        pre_array = [1,1,1,1,tp,tp,tp,tp,tp,tp,tp**2,tp**2]
    for itype in range(b2ps):
        for jtype in range(b2ps):
            #print(f"itype = {itype}, jtype = {jtype}, corr = {v_q0[0,0,itype,jtype]}");
...
def j2j2_wen(path):
    ...
    for itype in range(b2ps):
        for jtype in range(b2ps):
            #print(f"wen, itype = {itype}, jtype = {jtype}, corr = {v_q0[0,time_slice,itype,jtype]}");
            accum += pre_array[itype]*pre_array[jtype]*v_q0[:,time_slice,itype,jtype]
...
def jj2_jxding(path):
    ...
    #sum over all locations i.e. take q=0
    v_q0 = jj2.sum((-1,-2))  #shape = -1, L, bps, hop2ps
