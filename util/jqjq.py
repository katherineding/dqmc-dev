...
def get_component(path,name="j2j2"): 
    ...
    elif name == "jnj" or name == "jjn" :
        ns, correlator = util.load(path, "meas_uneqlt/n_sample",\
            f"meas_uneqlt/new_{name}")

        # twp phases
        correlator.shape = -1, L, bps, bps, Ny, Nx

    elif name == "jnjn" or name == "jj" :
        ns, correlator = util.load(path, "meas_uneqlt/n_sample", \
            f"meas_uneqlt/{name}")
...
def thermal_sum(path,q0_corrs):
    ...
    pre_arr = np.array([1,-U,U+2*mu]) * 1/4 * 1/2
...
def my_correlators(path):
    ...
    """ 
    Given path, run get_component() and thermal_sum() to get 
    Dictionary with jj, JQJ, JJQ, JQJQ
    Return a dictionary with named elements.
    E.g. result["JQJ"].shape = (4, Nbin_complete, L)
    NOTE: elements devided by sign for prettier plotting
    """
    ...
    # return jqjq_dict#, jqj_dict, jjq_dict
...
#             "metadata/t'","params/N","params/L","params/dt")    
#     ns, s, j2j2, jnj, jjn, jnjn = util.load(path, 
#         "meas_uneqlt/n_sample", "meas_uneqlt/sign", 
#         "meas_uneqlt/j2j2","meas_uneqlt/jnj","meas_uneqlt/jjn",
#         "meas_uneqlt/jnjn")

#     #jnjn.measurement same as my jnjn terms
#     jnjn.shape= -1, L, bps, bps, Ny, Nx
#     #jnj measurement contains jn-j, jn-j2
#     jnj.shape = -1, L, b2ps, bps, Ny, Nx
#     #jjn measurement contains j-jn, j2-jn
#     jjn.shape = -1, L, bps, b2ps, Ny, Nx
#     #j2j2 measurement really contains j-j, j-j2, j2-j, j2-j2 types 
#     j2j2.shape= -1, L, b2ps, b2ps, Ny, Nx

#     dm_dict, de_dict = da.eqlt_meas_1(path,['density'])
#     dm = dm_dict["density"].real
#     de = de_dict["density"]
#     tt = fr"{Nx}x{Ny} n={dm.real:.3g} t'={tp} U={U} $\beta$={beta:.3g}"
#     print(f"Achieved n={dm:.3g} , SE(n) =  {de:.3g}")

#     #use only completed bins
#     mask = ns == ns.max(); nbin = mask.sum()
#     ns, s, j2j2, jnj, jjn, jnjn = ns[mask], s[mask], \
#         j2j2[mask], jnj[mask],jjn[mask],jnjn[mask]

#     #NOTE: no error analysis
#     jnjn_q0 = jnjn.sum((-1,-2))/np.mean(s)
#     jjn_q0  = jjn.sum((-1,-2))/np.mean(s)
#     jnj_q0  = jnj.sum((-1,-2))/np.mean(s)
#     j2j2_q0 = j2j2.sum((-1,-2))/np.mean(s)

#     #bond type defn same for all three methods
#     t_arr = [1,1,tp,tp]
#     # 2bond type
#     t2_arr=[4*tp, 4*tp,  2,  2,  1+2*tp**2, 1+2*tp**2, \
#              2*tp, 2*tp, tp**2, 2*tp, 2*tp, tp**2]
...
#     #=========================================================================
#     #prefactors same for all three methods
#     pre_arr = 1/16*np.outer([1,-U,U+2*mu],[1,-U,U+2*mu])
#     xx = pre_arr[0,0]*j2j2_xx + pre_arr[0,1]*j2jn_xx + pre_arr[0,2]*j2j_xx + \
#          pre_arr[1,0]*jnj2_xx + pre_arr[1,1]*jnjn_xx + pre_arr[1,2]*jnj_xx + \
#          pre_arr[2,0]*jj2_xx  + pre_arr[2,1]*jjn_xx  + pre_arr[2,2]*jj_xx 
...
#     #==========================================================================
#     #prefactors same for all three methods for JQ(tau)J(0)
#     #TODO: check if this is actually consistent with bond, map definitions
#     pre_arr = np.array([1,-U,U+2*mu]) * 1/4 * 1/2
#     xx = pre_arr[0]*j2j_xx + pre_arr[1]*jnj_xx + pre_arr[2]*jj_xx
#     yy = pre_arr[0]*j2j_yy + pre_arr[1]*jnj_yy + pre_arr[2]*jj_yy
